...
function sendMessage() {
  ...
  if (localStorage.getItem("isSent") == "true") {
    createToast(
      {
        title: "Unsuccessful",
        description: "You've already sent a message, wait a few minutes",
      },
      {
        type: "warning",
    ...
    u_Name.value.trim().length > 0 &&
    u_Mail.value.trim().length > 0 &&
    ...
    emailjs
      ...
      .then((res) => {
        ...
        createToast(
          { title: "Successful", description: "Your message succesfully sent" },
          {
            type: "success",
            position: "bottom-left",
            showIcon: true,
      ...
      .catch(() => {
        createToast(
          { title: "Unsuccessful", description: "Your request failed" },
          {
            type: "warning",
            position: "bottom-left",
            showIcon: true,
    ...
    createToast(
      { title: "warning", description: "There is unfilled areas" },
      {
        type: "warning",
        position: "bottom-left",
        showIcon: true,
...
<template>
  <section class="scrollSettings h-full w-full space-y-8 pt-12 pb-10">
    ...
    <form
      @submit="sendMessage"
      action=""
      class="text-gray-300 w-full space-y-5"
    >
      <div class="flex space-x-6 px-5 sm:flex-col sm:space-x-0 sm:space-y-5">
        <input
          class="w-1/2 rounded-full sm:w-full"
          type="text"
          placeholder="Name"
          minlength="1"
          maxlength="30"
      ...
      <div class="px-5 relative">
        <textarea
          ...
          required
        ></textarea>
        <div class="absolute right-8 bottom-4 text-gray-500">
          {{ messageCount }}/1000
        </div>
      </div>
